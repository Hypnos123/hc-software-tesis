...
export class InterfazChatComponent implements OnDestroy {
  ...
  private readonly initialMessage = 'Hola, soy tu asistente virtual. Puedes preguntarme sobre pacientes, historias clínicas, consultas, especialidades o tipos de enfermedad. También puedo ayudarte a verificar si un paciente ya está registrado por DNI o nombre completo antes de crear una historia clínica.';
  private activeRequest?: Subscription;
  private logoutSubscription: Subscription;
  isOpen = false; userMessage = ''; isLoading = false;
  messages: ChatMessage[] = this.getInitialMessages();
  quickQuestions = ['Buscar paciente por DNI','Buscar paciente por nombre','Verificar posible duplicado','Historias clínicas creadas hoy','Pacientes registrados este mes','Consultas incompletas','Consultas pendientes','Consultas atendidas hoy','Especialidad más requerida','Enfermedad más registrada'];
  ...
  quickAsk(text: string): void { if (this.isLoading) return; const prompt = this.duplicatePrompt(text); if (prompt) { this.userMessage = prompt; return; } this.userMessage = text; this.sendMessage(); }
  ...
  private duplicatePrompt(text: string): string | null { if (text === 'Buscar paciente por DNI') return 'Busca si existe el paciente con DNI '; if (text === 'Buscar paciente por nombre') return 'Verifica si '; if (text === 'Verificar posible duplicado') return 'Verifica si existe una historia clínica para '; return null; }
