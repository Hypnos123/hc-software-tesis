...
export class InterfazChatComponent implements OnDestroy {
  ...
  minimizeChat(): void { this.saveScrollPosition(); this.autoFollowPresentation = false; this.isOpen = false; this.scheduleFloatingMessage(90_000); }
  closeChat(): void {
  ...
  private formatResponse(response: IAsistenteResponse): string {
    const text = response.respuesta || 'No pude identificar la consulta.';
    if (response.intencion !== 'BUSQUEDA_PACIENTE_DNI') return text;
    return text.replace(/\s*No se recomienda crear una nueva historia clínica para este paciente\.\s*$/u, '').trimEnd();
  }
}
