...
export class DetallesConsultasComponent implements OnInit {
  ...
  cargarDetalleConsulta(id: number): void { this.service.getConsultaById(id).subscribe(c => { if (!c) return; this.frm.patchValue({ nombreCompleto: [c.apellidos, c.nombres].filter(Boolean).join(' '), dni: c.numDocumento, edad: c.edad, enfermedadesPrevias: c.enfermedadesPrevias, cirugiasPrevias: c.cirugiasPrevias, alergiaMedicamentos: c.alergiaMedicamentos, presionArterial: c.presionArterial, frecuenciaCardiaca: c.frecuenciaCardiaca, frecuenciaRespiratoria: c.frecuenciaRespiratoria, talla: c.talla, temperatura: c.temperatura, peso: c.peso, fechaConsulta: this.toDate(c.fechaConsulta), tiempoEnfermedad: c.tiempoEnfermedad, tipoEnfermedad: c.tipoEnfermedad, especialidadRequerida: c.especialidadRequerida, idEmpleadoDoctor: c.idEmpleadoDoctor, relatoPaciente: c.relatoPaciente, diagnostico: c.diagnostico, examenesRecetados: c.examenesRecetados, receta: c.receta, tratamiento: c.tratamiento, proximaCita: this.toDate(c.proximaCita) }); if (this.modo === 'ver') this.frm.disable(); else this.deshabilitarDatosPaciente(); }); }
